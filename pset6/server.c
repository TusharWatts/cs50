...
//newly added
bool req_line_check(const char* , const char* );
bool method_check(char*);
bool req_target_check(const char*,const char* );
bool http_check(const char*);
...
const char* lookup(const char* path)
{
    ...
    {
        char ch='.';
        char* path_end=strchr(path,ch);
        
        //if the path ends in .css
        if(   strcasecmp(  path_end , ".css")  ==0  )
             return "text/css";
        
            
        //if the path ends in .html
        if(   strcasecmp(  path_end , ".html")  ==0  )
            return "text/html";
        
        //if the path ends in .gif
        if(   strcmp(  path_end , ".gif")  ==0  )
            return "image/gif";
        
        //if the path ends in .ico
        if(   strcmp(  path_end , ".ico")  ==0  )
            return "image/x-icon";
        
        //if the path ends in .jpg
        if(   strcmp(  path_end , ".jpg")  ==0  )
            return "image.jpeg";
        
        //if the path ends in .js
        if(   strcmp(  path_end , ".js")  ==0  )
            return "text/javascript";
        
        //if the path ends in .php
        if(   strcmp(  path_end , ".php")  ==0  )
            return "text/x-php";
        
        //if the path ends in .png
        if(   strcmp(  path_end , ".png")  ==0  )
            return "image/png";
    }
    return NULL;
}
 ...
 */
bool parse(const char* lin, char* abs, char* qry)
{
    // TODO
...
bool req_line_check(const char* line, const char* abs_path)
{
    char method[5];
    int p=0;
    while(line[p]!=' ')
    ...
    if(  (method_check(method))  &&  (req_target_check(method,abs_path))  &&  ( http_check(line)  )   )
        return true;
    else
        return false;
}

bool method_check(char* method)
{
    int ret=strcmp(method,"GET");
...
bool req_target_check(const char* method , const char* abs)
{
    if(    ( *(abs+0) == '/' )  && ( *(abs+0) != '"')      )
    {
        for(int k=0; *(abs+k)!=' ' ; k++)
...
bool http_check(const char* line)
{
    char ch=' ';
    char *temp=strchr(line,ch);
    char *temp1 = (temp+1) ;
    char *h=strchr(temp1,ch);
    char *http = (h+1);
    
    int ret=strcmp(http,"HTTP/1.1");
    if(ret==0)
        return true;
    else
    {
